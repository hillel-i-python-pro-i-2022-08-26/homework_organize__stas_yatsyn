...
def organize_data(humans: T_HUMANS) -> dict[str, list]:
    ...
    """
    list_of_group_and_names: dict[str, list] = defaultdict(list)
    for i in humans:
        list_of_group_and_names[i["group"]].append(i["name"])

    return list_of_group_and_names
...
def get_formatted_output(data: dict) -> str:
    ...
    return "\n".join(
        [
            f'В компанії "{group}" працюють: {", ".join(names)}. ' f"Всього: {len(names)} працівник(а) "
